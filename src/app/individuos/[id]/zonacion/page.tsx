...
export default function ZonacionFichaPage() {
  ...
  const existing = fichas.find((f) => f.tipo === "zonacion");
  ...
  async function handleSave(payload: {
    ...
    try {
      if (existing) {
        ...
        await crear({ individuoId: id, tipo: "zonacion", ...payload });
      }
  ...
  return (
    <div className="space-y-5">
      <nav className="text-sm text-muted">
        ...
        <span className="px-2 text-faint">/</span>
        <span className="text-ink">Zonación</span>
      </nav>
