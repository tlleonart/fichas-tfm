...
export default function NuevoIndividuoPage() {
  const router = useRouter();
  const crear = useMutation(api.individuos.crear);
  ...
  const required = form.sitio && form.numeroFosa && form.codigoUF && form.numeroIndividuo;
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <header>
        <h1 className="font-serif text-3xl font-semibold text-ink">Nuevo individuo</h1>
        <p className="mt-1 text-sm text-muted">
          La identidad estructurada es la clave que vincula los métodos. El código
          canónico se genera automáticamente.
        </p>
      </header>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6 p-6">
        ...
        <div className="flex justify-end gap-3">
          <Link href="/individuos" className="btn btn-ghost">
            Cancelar
          </Link>
          <button type="submit" className="btn btn-primary" disabled={saving || !required}>
            {saving ? "Creando…" : "Crear individuo"}
          </button>
        </div>
      </form>
    </div>
